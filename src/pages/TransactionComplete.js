...
export default class TransactionComplete extends Component {
    ...
    componentWillMount() {
        ...
        axios.post(`http://localhost:3001/transaction-complete`, formData, {
        })
            .then((response) => {
                this.setState({
    ...
    render() {
        ...
        const transactions = this.state.transactions.map((item, index) => {
            return <tr>
                ...
                <td>{item.total}</td>
                <td><Link to={`/detail-transaction-complete/${item.id_user}/${item.id_transaction}`}><Button variant="secondary">See Details</Button>{' '}</Link></td>
            </tr>
        })
